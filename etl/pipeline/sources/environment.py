...
class EnvironmentSource:
    ...
    async def fetch(self, client: httpx.AsyncClient) -> list[dict]:
        ...
        try:
            ...
            weather_task = client.get(
                OWM_WEATHER_URL,
                params={"lat": PARIS_LAT, "lon": PARIS_LON, "appid": OWM_API_KEY, "units": "metric"},
            )
            ...
            if not isinstance(weather_resp, Exception):
                ...
                env_record.update({
                    "temperature_celsius": weather_data.get("main", {}).get("temp", 0),
            ...
            logger.info("✅ Données environnementales récupérées — AQI: %s", env_record.get("aqi"))

        except Exception as e:
            logger.error("❌ OpenWeatherMap: %s", str(e))
            results = self._generate_simulated_environment()
            logger.warning("⚠️  %d enregistrements environnementaux simulés", len(results))
    ...
    def _generate_simulated_environment() -> list[dict]:
        ...
        return [{
            ...
            "latitude": PARIS_LAT,
            "longitude": PARIS_LON,
            "aqi": random.randint(3, 5) if is_pollution_peak else random.randint(1, 3),
            "pm25": random.uniform(20, 80) if is_pollution_peak else random.uniform(5, 25),
            "pm10": random.uniform(30, 100) if is_pollution_peak else random.uniform(10, 40),
            "no2": random.uniform(60, 200) if is_pollution_peak else random.uniform(10, 60),
            ...
            "precipitation_mm": random.uniform(0, 5) if random.random() > 0.7 else 0,
        }]
